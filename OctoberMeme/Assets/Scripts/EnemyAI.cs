...
public class EnemyAI : MonoBehaviour
{
    ...
    {      
        ...
        if(Physics.Raycast(transform.position, transform.forward, out hit, lookCheckDistance))
        {
            //if its the player then chase the player
            if(hit.transform.TryGetComponent<PlayerMove>(out PlayerMove move))
            {
                return true;
            }
        }
